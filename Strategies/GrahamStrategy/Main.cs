    ...
    public class GrahamStrategy : QCAlgorithm
    {
        ...
        private void AnalyzeFundamentals()
        {
            ...
            foreach (var kvp in Securities)
            {
                ...
                try
                {
                    // Skip benchmark and non-equity securities
                    if (security.Type != SecurityType.Equity) continue;

                    // For now, we'll use simplified criteria based on price action
                    // In a real implementation, you would need to access fundamental data through
                    // universe selection or other LEAN fundamental data methods
                    
                    if (ApplySimplifiedCriteria(symbol, security))
                    {
                        eligibleSymbols.Add(symbol);
                        Log($"Security {symbol} passed simplified criteria");
                    }
                }
                catch (Exception ex)
                {
            ...
            Log($"After criteria filtering: {eligibleSymbols.Count} securities passed");
        ...
        /// <summary>
        /// Apply simplified criteria based on price and volume
        /// </summary>
        private bool ApplySimplifiedCriteria(Symbol symbol, Security security)
        {
            try
            {
                // Basic price filter
                if (security.Price <= 5m) return false;

                // Check if we have enough price history
                if (!security.HasData) return false;

                // For demonstration, we'll use a simplified approach
                // In a real implementation, you would access fundamental data properly
                
                // Store simplified security data
                _securityData[symbol] = new SecurityData
                {
                    Symbol = symbol,
                    PE = 15m, // Default/estimated values
                    PB = 2m,
                    CurrentRatio = 1.5m,
                    DebtToEquity = 0.3m,
                    ROE = 0.12m,
            ...
            {
                Log($"Error applying criteria to {symbol}: {ex.Message}");
                return false;
            }
        }
