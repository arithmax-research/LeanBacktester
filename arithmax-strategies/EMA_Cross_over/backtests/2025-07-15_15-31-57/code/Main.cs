...
namespace QuantConnect.Algorithm.CSharp
{
    public class EMACrossover : QCAlgorithm
    {
        ...
        /// OnEndOfDay event called at the end of each trading day
        public override void OnEndOfDay()
        {
            // Calculate and log portfolio statistics
            var totalValue = Portfolio.TotalPortfolioValue;
            var totalReturn = (totalValue - 100000) / 100000;
            
            // Log daily portfolio performance
            Plot("Portfolio", "Total Value", totalValue);
            Plot("Portfolio", "Total Return %", totalReturn * 100);
            
            // Log individual EMA values for monitoring
            foreach (var symbol in _symbols)
            {
                if (_emaIndicators[symbol].IsReady)
                {
                    ...
                    Plot($"EMA_{symbol}", "Price", price);
                    Plot($"EMA_{symbol}", "EMA", ema);
                }
            }
        }
