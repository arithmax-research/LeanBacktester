...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class GrahamStrategy : QCAlgorithm
    {
        ...
        public override void Initialize()
        {
            ...
            Schedule.On(DateRules.EveryDay(), TimeRules.At(15, 30), RiskManagement);
            ...
            foreach (var stock in fine)
            {
                if (ApplyGrahamCriteria(stock))
                {
                    ...
                    _securityData[stock.Symbol] = new SecurityData
                    {
                        ...
                        CurrentRatio = stock.OperationRatios.CurrentRatio.Value,
                        DebtToEquity = stock.OperationRatios.TotalDebtEquityRatio.Value,
                        ROE = stock.OperationRatios.ROE.Value,
                        ProfitMargin = stock.OperationRatios.NetMargin.Value,
                        MarketCap = stock.MarketCap,
                        Sector = stock.AssetClassification.MorningstarSectorCode,
                        Industry = stock.AssetClassification.MorningstarIndustryCode,
                        Beta = stock.ValuationRatios.Beta,
                        Revenue = stock.IncomeStatement.TotalRevenue.Value,
                        DividendYield = stock.ValuationRatios.DividendYield
                    };
                }
            }
        ...
        private bool ApplyGrahamCriteria(FineFundamental stock)
        {
            try
            {
                ...
                var revenue = stock.IncomeStatement.TotalRevenue.Value;
        ...
        private IEnumerable<Symbol> ApplyDiversification(List<ScoredSymbol> scoredSymbols)
        {
            ...
            var sectorCounts = new Dictionary<MorningstarSectorCode, int>();
            ...
            foreach (var item in scoredSymbols)
            {
                ...
                var currentSectorCount = sectorCounts.GetValueOrDefault(sector, 0);
        ...
        {
            var sectorExposure = new Dictionary<MorningstarSectorCode, decimal>();
            var totalValue = Portfolio.TotalPortfolioValue;
            ...
            foreach (var kvp in sectorExposure)
            {
                ...
                {
                    Log($"Sector concentration warning: {kvp.Key} exposure is {kvp.Value:P2}");
                }
            }
        }
    ...
    public class SecurityData
    {
        ...
        public MorningstarSectorCode Sector { get; set; }
        public MorningstarIndustryCode Industry { get; set; }
