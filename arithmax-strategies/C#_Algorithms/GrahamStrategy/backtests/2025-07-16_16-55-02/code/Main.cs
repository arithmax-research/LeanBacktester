...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class GrahamStrategy : QCAlgorithm
    {
        ...
        public override void Initialize()
        {
            ...
            // Define specific stocks to analyze
            _stockList = new string[] 
            {
                "AAPL", "MSFT", "AMZN", "GOOGL", "GOOG", "BRK.B", "JNJ", "UNH", "XOM", "NVDA", 
                "TSLA", "PG", "MA", "HD", "V", "JPM", "DIS", "BAC", "ADBE", "CMCSA", 
                "INTC", "PFE", "WMT", "COST", "KO", "CSCO", "PEP", "CRM", "ABBV", "T", 
                "MRK", "ACN", "AVGO", "NFLX", "CVX", "TXN", "ORCL", "NKE", "LLY", "MCD", 
                "QCOM", "MDT", "NEE", "BMY", "LOW", "UPS", "PM", "AMGN", "RTX", "UNP", 
                "INTU", "C", "LIN", "GS", "AMD", "AXP", "IBM", "GE", "BLK", "SBUX", 
                "ISRG", "SPGI", "CCI", "FIS", "MO", "PLD", "DE", "ADP", "TMUS", "CB", 
                "DUK", "MDLZ", "BDX", "MMM", "SO", "EL", "CI", "CL", "ITW", "ICE", 
                "SHW", "MMC", "PNC", "EW", "REGN", "CSX", "ROP", "GM", "EMR", "BSX", 
                "VRTX", "ETN", "APD", "FCX", "DD", "DHR", "ADI", "NSC", "CTSH", "FDX", 
                "ATVI", "APH", "EA", "COF", "ILMN", "BIIB", "ALGN", "CHTR", "DLR", "MET", 
                "FTNT", "KLAC", "EXC", "CTAS", "CTXS", "IDXX", "ROST", "ODFL", "EBAY", "CE", 
                "PEG", "KEYS", "MTB", "TT", "CDNS", "CPRT", "ORLY", "SWKS", "FAST", "STZ", 
                "CDW", "DLTR", "PCAR", "TRV", "GRMN", "ANSS", "TTWO", "XYL", "ANET", "INCY", 
                "TECH", "FTV", "HSY", "VAR", "LDOS", "GLW", "ALXN", "DXCM", "MSCI", "EXPE", 
                "CPB", "CTRA", "JKHY", "AZO", "ULTA", "LYV", "MSI", "HLT", "CERN"
            };

            // Add securities manually
            foreach (var ticker in _stockList)
            {
                try
                {
                ...
                {
                    Log($"Failed to add security {ticker}: {ex.Message}");
                }
            }

            // Schedule fundamental data analysis
            Schedule.On(DateRules.MonthStart(), TimeRules.At(9, 0), AnalyzeFundamentals);
        ...
        /// <summary>
        /// Analyze fundamentals for all securities
        /// </summary>
        private void AnalyzeFundamentals()
        {
            if (IsWarmingUp) return;

            Log($"Analyzing fundamentals for {Securities.Count} securities");
            
            var eligibleSymbols = new List<Symbol>();
            _securityData.Clear();
            ...
            foreach (var kvp in Securities)
            {
                ...
                try
                {
                    ...
                    var fundamental = Fundamentals[symbol];
                    if (fundamental != null)
                    {
                        if (ApplyGrahamCriteriaToSecurity(symbol, fundamental))
                        {
                            eligibleSymbols.Add(symbol);
                            Log($"Security {symbol} passed Graham criteria");
                        }
                    }
                    else
                    {
                        Log($"No fundamental data available for {symbol}");
                    }
                }
                catch (Exception ex)
                {
                    Log($"Error analyzing {symbol}: {ex.Message}");
                }
            }

            Log($"After Graham criteria: {eligibleSymbols.Count} securities passed");

            // Apply diversification and quality scoring
            var selectedSymbols = SelectDiversifiedPortfolio(eligibleSymbols);
        ...
        /// <summary>
        /// Apply Graham criteria to a security with fundamental data
        /// </summary>
        private bool ApplyGrahamCriteriaToSecurity(Symbol symbol, Fundamental fundamental)
        {
            try
            {
                ...
                if (security.Price <= 1m) return false;
                
                // Size filter - minimum market cap
                if (fundamental.MarketCap < 100000000) return false;

                // Earnings stability - positive earnings
                if (fundamental.EarningReports.BasicEPS.Value <= 0) return false;

                // PE ratio - reasonable valuation
                var pe = fundamental.ValuationRatios.PERatio;
                if (pe <= 0 || pe > 25) return false;

                // PB ratio - conservative book value
                var pb = fundamental.ValuationRatios.PBRatio;
                if (pb <= 0 || pb > 3.5) return false;

                // Current ratio - strong liquidity
                ...
                var debtToEquity = fundamental.OperationRatios.TotalDebtEquityRatio.Value;
                if (debtToEquity > 0.8) return false;

                // Profitability - positive profit margins
                var netMargin = fundamental.OperationRatios.NetMargin.Value;
                if (netMargin < 0.03) return false;

                // ROE - reasonable return on equity
                var roe = fundamental.OperationRatios.ROE.Value;
                if (roe < 0.08) return false;

                // Revenue - positive revenue
                var revenue = fundamental.FinancialStatements.IncomeStatement.TotalRevenue.Value;
                if (revenue <= 0) return false;

                // Store security data
                _securityData[symbol] = new SecurityData
                {
                    Symbol = symbol,
                    PE = (decimal)pe,
                    PB = (decimal)pb,
                    CurrentRatio = (decimal)currentRatio,
                    DebtToEquity = (decimal)debtToEquity,
                    ROE = (decimal)roe,
                    ProfitMargin = (decimal)netMargin,
                    MarketCap = fundamental.MarketCap,
                    Sector = (int)fundamental.AssetClassification.MorningstarSectorCode,
                    Industry = (int)fundamental.AssetClassification.MorningstarIndustryCode,
                    Beta = 1.0m,
                    Revenue = (decimal)revenue,
                    DividendYield = (decimal)fundamental.ValuationRatios.ForwardDividendYield
                };
            ...
            {
                Log($"Error applying Graham criteria to {symbol}: {ex.Message}");
                return false;
            }
        }
