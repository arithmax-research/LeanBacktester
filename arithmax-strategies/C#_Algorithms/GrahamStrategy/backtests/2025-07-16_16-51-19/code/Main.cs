...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class GrahamStrategy : QCAlgorithm
    {
        ...
        public override void Initialize()
        {
            ...
            // Add universe selection with specific stocks
            AddUniverse(coarse => CoarseSelectionFilter(coarse, stockList), FineSelectionFilter);
        ...
        /// <summary>
        /// Coarse universe selection - filter specific stocks
        /// </summary>
        private IEnumerable<Symbol> CoarseSelectionFilter(IEnumerable<CoarseFundamental> coarse, string[] stockList)
        {
            var coarseCount = coarse.Count();
            Log($"Coarse universe selection: Processing {coarseCount} securities");

            // Convert stock list to symbols
            var targetSymbols = stockList.Select(x => QuantConnect.Symbol.Create(x, SecurityType.Equity, Market.USA)).ToHashSet();

            var filtered = coarse
                .Where(x => targetSymbols.Contains(x.Symbol) && 
                           x.HasFundamentalData && 
                           x.Price > 1m && 
                           x.DollarVolume > 10000)
                .Select(x => x.Symbol);

            var filteredCount = filtered.Count();
            Log($"After coarse filtering: {filteredCount} securities from target list found with fundamental data");
